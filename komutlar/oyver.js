...
if(delay+3600000 > Date.now()){
 let zaman = moment.duration(delay + 3600000 - Date.now()).format("[**] D [**Gün**] H [**Saat**] m [**Dakika**] s [**Saniye]")
 if(delay) return message.channel.send("? Bu sunucuya oy verebilmek için " + zaman + " beklemen gerekiyor.")
} else {
...
.setID("rast2")
// -------- \\
const doðru = new disbut.MessageButton()
.setStyle("green")
.setLabel(kelime)
.setID("doðru")
// -------- \\
...
// -------- \\
  ...
  const cevp = [new disbut.MessageActionRow().addComponents(rast1, rast2, doðru, rast3), new disbut.MessageActionRow().addComponents(rast1, rast2, rast3, doðru), new disbut.MessageActionRow().addComponents(rast1, doðru, rast2, rast3), new disbut.MessageActionRow().addComponents(doðru, rast1, rast3, rast2)]
  var weq  = cevp[Math.floor(Math.random() * cevp.length)];

// -------- \\
 
await message.channel.send("<@!" + message.author.id +'> Resimdeki kodu seç. Ýþlem **30** saniye sonra iptal edilecek.', {
  components: weq,
  embed: embed
}).then(m => { 
...
// -------- \\
 ...
 message.channel.send("> ? **Baþarýlý!** <@!"+ message.author +"> Sunucuya oy verdiniz! Sunucunun toplam oyu: `"+ data +"` .") 
...
// -------- \\
      ...
      message.channel.send("> ? **Hata!** <@!"+ message.author +"> Yanlýþ butonu seçtiniz.")  
      m.delete()
